...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalInvest = Order::where('user_id',$user->id)->where('status', '!=', 0)->sum('amount');

        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','totalWithdraw','totalTransaction', 'totalInvest'));
    }
    ...
    public function invests(Request $request, $id)
    {
        ...
        $page_title = 'Invests logs of ' . $user->username;
        $invests = $user->orders()->latest()->paginate(getPaginate());
        $empty_message = 'No invest yet';
        return view('admin.reports.invests', compact('page_title', 'user', 'invests', 'empty_message'));
    }

    public function coinBalance(Request $request, $id)
    {
        $user = User::findOrFail($id);

        $page_title = 'Coin Balance List of ' . $user->username;
        $coins = $user->coinBalances()->latest()->paginate(getPaginate());
        $empty_message = 'No data found';

        return view('admin.reports.coin_balance', compact('page_title', 'user', 'coins', 'empty_message'));
    }
    ...
    public function withdrawals(Request $request, $id)
    {
        ...
        $userId = $user->id;
        $action = true;
        return view('admin.withdraw.withdrawals', compact('page_title', 'user', 'withdrawals', 'empty_message','userId', 'action'));
    }
