                            ...
                            @forelse($transactions as $trx)
                                <tr>
                                    ...
                                    <td data-label="@lang('Amount')" class="budget">
                                        <strong @if($trx->trx_type == '+') class="text-success" @else class="text-danger" @endif> {{($trx->trx_type == '+') ? '+':'-'}} {{getAmount($trx->amount)}} {{__($trx->currency??$general->cur_text)}}</strong>
                                    </td>
                                    <td data-label="@lang('Charge')" class="budget">
                                        {{ getAmount($trx->charge) }} {{ __($trx->currency??$general->cur_text) }}
                                    </td>
                                    <td data-label="@lang('Post Balance')">{{ getAmount($trx->post_balance) }} {{ __($trx->currency??$general->cur_text) }}</td>
