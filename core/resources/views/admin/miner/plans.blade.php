                        ...
                        <tbody class="list">
                            @forelse($plans as $plan)
                                <tr>
                                    ...
                                    <td data-label="@lang('Price')"> {{ getAmount($plan->price) }} </td>
                                    <td data-label="@lang('Speed')"> {{ $plan->speed }} {{ $plan->speedUnitText}} </td>
                                    <td data-label="@lang('Period')"> {{ $plan->period }} {{ $plan->periodUnitText}}</td>
                                    <td data-label="@lang('Return /Day')"> {{ $plan->returnPerDay }} {{ $plan->miner->coin_code }} </td>
                                    ...
                                    <td data-label="@lang('Action')">
                                        <a href="javascript:void(0)"
                                            data-toggle="tooltip" data-placement="top" title="@lang('Edit')" data-plan="{{ $plan }}" data-toggle="modal"
...
@push('breadcrumb-plugins')
    <button class="btn btn-sm btn--success box--shadow1 text--small add-btn"> <i class="las la-plus"></i> @lang('Add New')</button>
    <a href="{{ route('admin.miner.index') }}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="fa fa-fw fa-backward"></i>Go Back</a>
@endpush
    ...
    <script>
        ...
        (function($){
            ...
            $('.add-btn').on('click', function () {
                var modal       = $('#addModal');
                modal.find('select[name=miner]').val(`{{ $this_miner->id }}`);
                modal.modal('show');
            });
